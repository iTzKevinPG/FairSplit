...
export function AuthCard() {
  ...
  return (
    <SectionCard
      ...
      description={
        fixedCodeMode
          ? 'Ingresa tu correo y el codigo fijo compartido para guardar tus datos.'
          : 'Recibe un codigo por email (o ingresa el codigo fijo si te lo compartieron) para guardar tus datos en backend.'
      }
      actions={
        canToggle ? (
          <button
    ...
    >
      {authenticated ? (
        <div className="flex items-center justify-between rounded-md border border-slate-200 bg-[color:var(--color-surface)] px-3 py-2 text-sm text-[color:var(--color-text-main)] dark:border-slate-700 dark:bg-[color:var(--color-app-bg)]">
          <span>Sesion activa con {email}</span>
      ...
      {showContent ? (
        <div className="flex flex-col gap-3">
          ...
          {fixedCodeMode ? (
            <div className="rounded-md border px-3 py-2 text-sm text-amber-900 dark:border-amber-500/40 dark:bg-amber-500/10 dark:text-amber-400">
              <p className="font-semibold">Modo codigo fijo</p>
              <p>Usa el codigo compartido.</p>
            </div>
          ) : (
          ...
          <label className="flex flex-col gap-1 text-sm text-[color:var(--color-text-muted)]">
          {fixedCodeMode ? 'Codigo' : 'Codigo de 6 digitos'}
          <input
            type="text"
            inputMode="numeric"
            value={code}
